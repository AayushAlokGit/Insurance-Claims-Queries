...
3. **`./design-decisions.md`** — numbered, accepted design decisions (**DD-001 … DD-023**, with gaps where decisions were withdrawn). Each entry has a rationale and what it commits us to. If a fresh idea conflicts with one of these, the DD wins until explicitly re-litigated.
...
6. **`./docs/extractor.md`** — Extractor stage deep dive. The hybrid rule-vs-LLM principle, the `Extractor` interface, the active extractor catalog (Reserve, Appointment LLM, RTW) with triggers / output schemas / rationale, the four LLM contract principles (schema-constrained output, discriminated-union empty case, required `evidence_quote` with substring check, explicit negative prompt rules), multi-extractor coordination on a single note, and failure modes.
...
- **Strict YAGNI on the domain model (DD-012).** The `Claim` table has 7 columns, not 12; the event taxonomy has 3 active types, not 5+. Speculative fields (`loss_description`, `body_parts`, `diagnoses`, `avg_weekly_wage`, `comp_rate`, `work_status_change`) were explicitly removed. Do not propose adding any of these back without naming the query that needs them.
...
- **Event-centric model.** Everything dated and meaningful is an `Event` row with `event_type`, `event_date`, `attributes` (JSON), `extraction_method`. The three active types are `reserve_change`, `appointment`, `return_to_work`. Future types are sketched in `data-modeling.md §4.5`.
- **Discriminated unions over nullable scalars.** Q1 returns `{status: 'returned' | 'pending', ...}`, not `days_to_rtw: number | null`. A `WHERE NOT NULL` at corpus scale silently drops populations and lies.
...
- **Prefers discriminated unions and explicit evidence requirements** over `null`-laden return types and inferred state.
