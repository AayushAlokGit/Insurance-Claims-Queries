...
STATUS:
- attended  — the visit happened. A medical-record block (a `Date of Appointment: X`, `Date of appointment: X`, or `DOS: X` line — with ANY number of leading dashes/underscores/whitespace or none, e.g. `---Date of Appointment:`, `--Date of Appointment:`, `__\nDate of appointment:` — followed by clinical content) ALWAYS counts as attended even without an "attended" verb, and even when the note's opening line is an administrative preamble that describes HOW the clinical record reached the file. The preamble verb does not matter — ALL of the following preambles still mean the templated block below IS the visit and MUST be emitted:
    • `Note: TCM received medical records. Reviewed and saved to file.`
    • `Note: TCM reviewed PT notes on OCCM portal. Saved to file.`
    • `Note: TCM reviewed OCCM Clinical Review. Saved to file.`
    • `Note: TCM pulled initial eval for outpatient PT from OCCM portal. Reviewed and saved to file.`
    • `Note: TCM received faxed records. Reviewed and saved to file.`
  The pattern is preamble-about-record-handling + templated medical block + clinical content. Generalize beyond the literal examples — any verb describing the TCM ingesting / reviewing / saving an external clinical record qualifies. "Clinical content" includes ANY of these section labels: `Plan:`, `Subjective:`, `Objective:`, `Assessment:`, `Assessment/Diagnosis:`, `Diagnosis:`, `Diagnosis & Assessment:`, `FCE Summary:`, `EMG/NCS report:`, `Interpretation:`, `Operative Note:`, `Therapeutic Exercise:`, `Discharge Summary:`, `Findings:`, `Impression:`, `Current complaints:`, or any free-text paragraph describing what happened at the visit.
...
DO NOT EXTRACT:
- Pure communications without a visit-action verb (emails, texts, fax cover sheets, records-receipt notices) when they are the ENTIRE content of the note. (When followed by a templated medical-record block, see the `attended` rule above — the templated block is the event.)
